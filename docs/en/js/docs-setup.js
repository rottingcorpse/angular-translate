NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$scope $translate $translateprovider able about accordingly actually add adding after all als also always an and anf angular angular-translate another any app apps are argument around as asynchronous at attribut attribute available awesome back bad basic basically be becomes been beginners belongs bin but button button_lang_de button_lang_en buttons by calls can cannot case cases certain change changelanguage choose code com combination come comes components config controller controllers controls cool corresponding course covered ctrl current currently de decide default detail deutsch different directive displaying do does doesn dynamic each ein either empty en englisch english ernsthaft et expect expects extend fall fallbacklanguage feature feeling find first for function functionality german get getting given good great greeting gro guide ha hallo happen has hasn have headline hello hey hi how however html ich ids if implement in incomplete instead interesting internally interpolated interpolation interpoliert into introduced invokes is it its just key know knows language languages last later learn learned learning least let level like limit ll loaded loaders loading looks love make many method missing missing_translation modul module more multi myapp name need needed new next ng-click ng-controller no not now oder of on one oops or our out overview paragraph pascalprecht pass passed passed_as_attribute passed_as_interpolation passed_as_text passing points possible practice prefer preferredlanguage pretty provide provides rather ready really recognize registered registers registration replacement returns runtime same sample say scope second service setting setup should simple since so some srsly started storage strings support switch switching table tables teach teaching tell tells text than that the them then there these things this through time to translate translate-values translated translation translations translationsde translationsen try trying turns two unformatted unfortunately unicorn until update usage use used uses using value values var variable_replacement ve very voil want was we well welt what when where whereas which will with within work world would wurde yet you your"
    },
    {
      "section": "guide",
      "id": "08_storages",
